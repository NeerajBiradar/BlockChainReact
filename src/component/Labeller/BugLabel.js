...
const BugLabel = () => {
    ...
    useEffect(() => {
        async function Connection() {
            ...
            let temp = await window.contract.methods.SendBugReport().call();
            temp = temp.filter((val, ind) => {
                return val.priority === 'default'
            });
            console.log(temp, "Nodefault");
            setData(temp);
            $(function () {
    ...
    const SendBugReport = async () => {
        for (let i = 0; i < data.length; i++) {
            const priority_value = document.getElementById('bug-priority' + i+1).value;
            if (priority_value !== '') {
                setCount(true);
        ...
        if(count === true){
            const result = await window.contract.methods.SetPrioritybug(BugArr, Bugpriority).send({ from: account });
            setTransactionHash(result.transactionHash);
            alert('Transaction Successful');
        }
        else{
            alert('No priority to submit');
        }
    ...
    return (
        <div className="container">
                    ...
                    {data.reverse().map((val, ind) => {
                        return (
                            <tr key={ind}>
                                ...
                                <td>
                                    {<div>
                                        <select className="form-select" id={"bug-priority" + ind + 1}>
            ...
            <input className="btn btn-primary m-2" type="submit" value="Bug Priority" onClick={SendBugReport} />
            {transactionHash && (
